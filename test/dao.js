...
describe("DAO", () => {
  ...
  it("has a working getSchemes() function to access its schemes", async () => {
    ...
    assert.equal(
      scheme.address,
      wrapperService.allContracts.GlobalConstraintRegistrar.address
    );
    ...
    assert.equal(
      scheme.address,
      wrapperService.allContracts.SchemeRegistrar.address
    );
    assert.isTrue(!!scheme.contract, "contract must be set");
    assert.isTrue(!!scheme.address, "address must be set");
    ...
    assert.equal(
      scheme.address,
      wrapperService.allContracts.UpgradeScheme.address
    );
    assert.isTrue(!!scheme.contract, "contract must be set");
    assert.isTrue(!!scheme.address, "address must be set");
