...
describe("Wallet", () => {
  ...
  it("can be encrypted and decrypted", async function () {
    ...
    const encryptedJSON = await wallet.encrypt("Passw0rd", progress => {
      process.stdout.write(".");
    });
    console.log("\n");
    console.log("Decrypting wallet");
    const wallet2 = await Wallet.fromEncrypted(
      encryptedJSON,
      "Passw0rd",
      progress => {
        process.stdout.write(",");
  ...
  it("can send and receive org tokens", async function () {
    ...
    const wallet1 = Wallet.fromPrivateKey(
      "0x0191ecbd1b150b8a3c27c27010ba51b45521689611e669109e034fd66ae69621"
    ); // This is the private key of accounts[0] in the daostack ganache-cli configuration
    ...
    assert.equal(await wallet1.getOrgTokenBalance(org.avatar.address), 100000);

    await wallet1.sendOrgTokens(
      org.avatar.address,
      wallet2.getPublicAddress(),
      10
    );
    assert.equal(await wallet1.getOrgTokenBalance(org.avatar.address), 99990);
    assert.equal(await wallet2.getOrgTokenBalance(org.avatar.address), 100010);
  });
});
