...
contract('GlobalConstraintRegistrar', (accounts) => {
  ...
  it("proposeToAddModifyGlobalConstraint javascript wrapper should work", async () => {
    ...
    const globalConstraintParametersHash = await tokenCapGC.getParametersHash(organization.token.address,3141);
    await tokenCapGC.setParameters(organization.token.address,3141);

    const votingMachineHash = await organization.votingMachine.getParametersHash(organization.reputation.address, 50, true);
    await organization.votingMachine.setParameters(organization.reputation.address, 50, true);

    const globalConstraintRegistrar = await organization.scheme('GlobalConstraintRegistrar');
