...
export class DAO {
  ...
  /**
   * Computes the number of transactions that will be generated by `DAO.new` given the given options.
   * This function is returned by `TransactionService.registerActionTransactionCounter(DAO.new)`,
   * and may also call it directly.
   * @param options The options you are going to pass to `DAO.new`.
   */
  public static transactionsInNew: InNewTransactionsCounter = (options: NewDaoConfig) => {
    /**
     * one for forgeOrg, one for setSchemes, one for the default votingMachine params.
     * then one for each scheme's params, and one for each scheme that is not using the default votingMachine
     */
    ...
    return 3 + numSchemes + numSchemesWithDefaultParams;
  };
...
/**
 * Type of the function returned by `TransactionService.registerActionTransactionCounter(DAO.new)`
 * for if you want to do typing checking.
 */
export type InNewTransactionsCounter = (options: NewDaoConfig) => number;
