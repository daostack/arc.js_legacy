...
function getNetwork() {
  ...
  return (network && (network != "ganache")) ? "network" : null;
}

if (config.get("providerUrl").indexOf("infura") !== -1) {
  provider = new ethers.providers.InfuraProvider(getNetwork(), config.get("apiToken"));
} else if (config.get("providerUrl").indexOf("etherscan") !== -1) {
  provider = new ethers.providers.EtherscanProvider(getNetwork(), config.get("apiToken"));
} else {
  provider = new ethers.providers.JsonRpcProvider(config.get("providerUrl"), getNetwork());
}
...
export class Wallet {
  ...
  static async fromEncrypted(encryptedJSON, password, progressCallback) {
    const wallet = new Wallet();
    wallet.wallet = await ethers.Wallet.fromEncryptedWallet(
      encryptedJSON,
      password,
      progressCallback
    );
  ...
  async getOrgTokenBalance(organizationAvatarAddress, inWei) {
    inWei = inWei || false;
    const org = await Organization.at(organizationAvatarAddress);
    const balance = await org.token.balanceOf(this.getPublicAddress());
  ...
  async sendEther(toAccountAddress, numEther) {
    return await this.wallet.send(
      toAccountAddress,
      ethers.utils.parseEther(numEther.toString())
    );
  }
  ...
  async sendOrgTokens(organizationAvatarAddress, toAccountAddress, numTokens) {
    ...
    const transactionData = org.token.transfer.request(
      toAccountAddress,
      numTokens,
      { from: this.getPublicAddress() }
    ).params[0].data;
    ...
    const transaction = {
      ...
      chainId: ethers.providers.Provider.chainId[getNetwork()]
    };

    const transactionHash = await this.wallet.sendTransaction(transaction);
