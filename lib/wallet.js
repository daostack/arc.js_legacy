...
if (config.get('providerUrl').indexOf('infura') !== -1) {
  provider = new ethers.providers.InfuraProvider(ethNetwork, config.get('apiToken'));
} else if (config.get('providerUrl').indexOf('etherscan') !== -1) {
  provider = new ethers.providers.EtherscanProvider(ethNetwork, config.get('apiToken'));
} else {
  provider = new ethers.providers.JsonRpcProvider(config.get('providerUrl'), config.get('network'));
}
