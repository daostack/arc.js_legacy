...
export function getWeb3() {
  ...
  if (typeof window !== "undefined" && typeof window.web3 !== "undefined") {
    ...
    preWeb3 = new Web3(
      new Web3.providers.HttpProvider(config.get('daostack.providerUrl'))
    );
  }
...
export function getValueFromLogs(tx, arg, eventName, index = 0) {
  ...
  if (tx.logs[index].type !== "mined") {
    const msg = `getValueFromLogs: transaction has not been mined: ${
      tx.logs[index].event
    }`;
    throw new Error(msg);
  }
  ...
  if (!result) {
    const msg = `getValueFromLogs: This log does not seem to have a field "${arg}": ${
      tx.logs[index].args
    }`;
    throw new Error(msg);
  }
