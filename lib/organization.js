...
export class Organization {
  ...
  async schemes(name) {
    ...
    if (name) {
      return schemes.filter((s) => { return s.name === name; });
    } else {
      return schemes;
    }
  }
  ...
  async _getSchemes() {
    ...
    const schemesMap = new Map(); // <string, _daoSchemeInfoFromLogs>
    ...
    return Array.from(schemesMap.values()).map((s) => { return s.scheme; });
  }

  _daoSchemeInfoFromLogs(blockNumber, logIndex, scheme) {
  ...
  async _handleSchemeEvent(err, eventsArray, adding, arcTypesMap, schemesMap) // : Promise<void>
  {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const permissions = await this.controller.getSchemePermissions(schemeAddress);
      ...
      const daoSchemeInfoFromLogs = new this._daoSchemeInfoFromLogs(
        eventsArray[i].blockNumber,
        eventsArray[i].logIndex,
        schemeInfo
        // isInDao = adding
      );

      const lastFound = schemesMap.get(schemeAddress);

      const isNewer = !lastFound ||
        (daoSchemeInfoFromLogs.blockNumber > lastFound.blockNumber) ||
        ((daoSchemeInfoFromLogs.blockNumber == lastFound.blockNumber) && (daoSchemeInfoFromLogs.logIndex > lastFound.logIndex));
