...
import { GenesisScheme } from "./contracts/GenesisScheme";
...
export class DAO {
  ...
  static async new(opts) {
    ...
    if (opts.genesisScheme) {
      genesisScheme = await GenesisScheme.at(opts.genesisScheme);
    } else {
      genesisScheme = await GenesisScheme.deployed();
    }

    const result = await genesisScheme.forgeOrg(opts);

    const avatarAddress = result.getValueFromTx("_avatar", "NewOrg");

    await genesisScheme.setSchemes(Object.assign({ avatar: avatarAddress }, opts));

    return await DAO.at(avatarAddress);
  }
  ...
  static async getDAOstack(genesisSchemeAddress) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const event = genesisScheme.InitialSchemesSet({}, { fromBlock: 0 });
        let avatarAddress;
        /**
